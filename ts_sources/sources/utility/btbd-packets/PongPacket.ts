...
/**
 * Implementation of the PongPacket Packet type
 */
export class PongPacket extends Packet {
    ...
    constructor(master_address: Buffer, destination_address: Buffer, master_signature: Buffer, timestamp: number, packet_id: Buffer) {
        super(PacketType.PongPacket, master_address, destination_address, master_signature, timestamp);
        if (packet_id.length !== 8) throw new Error('Invalid Packet ID');
        this.PacketID = packet_id;
    }

    /**
     * Static method to build and check if a received raw PongPacket is valid
     *
     * @param header
     * @param body
     * @param timestamp
     */
    public static fromRaw(header: Buffer, body: Buffer, timestamp: number): PongPacket {
        ...
        const cp: PongPacket = new PongPacket(master_address, destination_address, master_signature, timestamp, packet_id);
        cp.Type = type;
        ...
        if (cp.Type !== PacketType.PongPacket) throw new Error('Invalid Packet Type detected');
